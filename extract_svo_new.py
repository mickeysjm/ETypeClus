...
def _get_objs_from_conjunctions(objs):
    ...
    for obj in objs:
        ...
        if contains_conj_strict(rightDeps, rightDepTags):
            more_objs.extend([tok for tok in rights if tok.dep_ in OBJECTS or tok.pos_ in NOUN_POS])
            if len(more_objs) > 0:
...
# resolve a 'who' where/if appropriate
def _get_who_resolution(toks):
    for tok in toks:
        if 'who' in [t.orth_ for t in tok.lefts]:
            return tok.head
    return None
...
def check_continuous(index_list):
    if (index_list[-1] - index_list[0] + 1) == len(index_list):
        return True
    else:
        return False
...
def expand(item, tokens, visited, first=True):
    if item.lower_ == 'that':
        ...
        if temp_item is not None:
            item = temp_item
    if item.lower_ == "who":
        temp_item = _get_who_resolution(tokens)
        if temp_item is not None:
            item = temp_item
    ...
    if hasattr(parts[-1], 'rights'):  # This logic is not very good, ideally we should check all the elements in the right
        for item2 in parts[-1].rights:
            if item2.pos_ == "DET" or item2.pos_ in NOUN_POS:
    ...
    if (not first) or check_continuous([p.i for p in parts]):
        return parts
    else:
        return list(item.subtree)
...
if __name__ == '__main__':
    ...
    # test_cases = [
    #     [-1, -1, "These conditions and peoples were blamed for the return of plague in India after 28 years."],
    # ]
    ...
    for test_case in test_cases:
        ...
        svos = findSVOs(doc)
        print("svos:", [[c[0] if c is not None else None for c in ele] for ele in svos])
        print("="*80)
